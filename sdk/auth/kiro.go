...
const kiroCredentialCopyCommand = `jq -c '{type:"kiro", authMethod:(.authMethod // "social"), region:(.region // "us-east-1"), accessToken, refreshToken, expiresAt, profileArn}' ~/.aws/sso/cache/kiro-auth-token-cli.json | pbcopy`
...
// To copy the latest local Kiro CLI credential JSON for management import, run:
//
//	jq -c '{type:"kiro", authMethod:(.authMethod // "social"), region:(.region // "us-east-1"), accessToken, refreshToken, expiresAt, profileArn}' ~/.aws/sso/cache/kiro-auth-token-cli.json | pbcopy
...
func (a KiroAuthenticator) Login(_ context.Context, _ *config.Config, _ *LoginOptions) (*coreauth.Auth, error) {
	return nil, fmt.Errorf("kiro: interactive login not supported; place kiro.json in auths/ directory or copy the latest local Kiro CLI credential with: %s", kiroCredentialCopyCommand)
}
